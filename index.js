...
function requireBearerToken(expected, envName) {
  return (req, res, next) => {
    if (!expected) return res.status(503).json({ error: `${envName} non configurato` });
    const header = req.get('authorization') || '';
    const provided = header.startsWith('Bearer ') ? header.slice(7) : '';
    if (!tokenMatches(provided, expected)) return res.status(403).json({ error: 'forbidden' });
    next();
  };
}
...
// Watchlist caricata esclusivamente dal backup Sofa Time locale o remoto.
...
const manifest = {
  ...
  version: '0.8.1',
  name: 'Sofa Time HUB',
  description: 'Sofa Time Hub - Addon Stremio/Nuvio per la tua watchlist Sofa Time',
  resources: ['catalog'],
  types: ['movie', 'series'],
  catalogs: [
...
async function main() {
  ...
  app.post('/backup-refresh', requireBearerToken(CLEAR_CACHE_TOKEN, 'CLEAR_CACHE_TOKEN'), async (req, res) => {
...
.token{width:100%;padding:12px;margin-bottom:1rem;border:1px solid #444;border-radius:8px;background:#101629;color:#eee}
...
<input class="token" type="password" id="token" autocomplete="current-password" placeholder="Token upload" />
...
b.addEventListener('click', () => {
  ...
  r.onload = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/upload-backup', { method: 'POST', body: e.target.result, headers: {'Content-Type':'application/octet-stream', 'Authorization':'Bearer ' + token} });
      const data = await res.json();
  ...
  app.post('/api/upload-backup', requireBearerToken(UPLOAD_TOKEN, 'UPLOAD_TOKEN'), express.raw({ type: 'application/octet-stream', limit: '20mb' }), async (req, res) => {
    try {
      let text = '';
      ...
      if (Buffer.isBuffer(zipBuffer) && zipBuffer.length > 4 && zipBuffer[0] === 0x50 && zipBuffer[1] === 0x4b) {
        ...
        if (zipEntries.length > 100) throw new Error('Archivio con troppi file');
        const totalSize = zipEntries.reduce((sum, entry) => sum + Number(entry.header.size || 0), 0);
        if (totalSize > 50 * 1024 * 1024) throw new Error('Archivio decompresso troppo grande');
      ...
      res.status(500).json({ error: 'Impossibile elaborare il backup' });
    }
  });

  // Cache-clear protetto da token
  app.post('/clear-cache', requireBearerToken(CLEAR_CACHE_TOKEN, 'CLEAR_CACHE_TOKEN'), (req, res) => {
...
module.exports = { writeFileAtomicSync, tokenMatches, idsFromStremioId, stremioIdFromIds, manifest };
